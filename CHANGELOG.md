...
- **A verifier catches only what the spec named, so a contract can now say what was never checked.** New `verification_contract` policy type (the 20th). The caller attaches the contract its work was specified against — `must_haves` and `prohibitions`, each carrying a verification tier and a spec-time `non_inferable` tag — and the policy disposes of the items that cannot be discharged: a violated item **blocks**, a test-tier item whose check never ran **blocks** (an unrun check is not evidence of anything), and an obligation the spec never settled returns **`insufficient_spec`** on a `require_approval` verdict rather than collecting a confident green.

  The measurement this answers (Sienkowski, *Verifier Reach Is Spec Reach*, 2026): with the relevant edge omitted from the spec, LLM verifiers produced a confident false pass on **100%** of runs [94–100], converting to a **98%** [91–100] catch once the same edge was written in. The blind spot is model-invariant and a ~30× spend increase recovers none of it — the verifier is not failing to reason, it is reasoning correctly over an input that does not contain the answer, and it reports the same confidence it uses when it is right. A confidence gate cannot help.

  Why it is not one of the existing types: `require_evidence` judges whether the act arrived with its evidence, `non_fabrication` judges outbound content against a source of truth, `green_contract` reads an observed build level. All three answer pass or fail. The new thing here is the third state — "the artifact does not contain the answer" — which is the only honest verdict on a blind spot and the one the measurement shows a verifier will otherwise never give.

  Escalation-only by construction: the validator refuses any disposition but `require_approval` or `block`, so a caller-declared contract can add a gate and never remove one, and a dishonest client gains at most the verdict it would have had with no contract at all. Off by default — no default pack ships it and `require_contract` is false, so every existing install is unchanged until an operator creates the policy on `/policies`, where it has a full rule builder (one control per disposition, with the strict defaults preselected). Surface budget amended 19 → 20 with the reason recorded in `THESIS.md`. (`app/lib/guard/verification-contract.ts`, `app/lib/guard/policy.ts`, `app/lib/validate.js`, `app/policies/components/PolicyRuleBuilderSection.tsx`, `app/policies/lib/policyFormModel.js`)

### Fixed

- **One NUL character could block an agent's tool calls and strand its ledger rows.** A governed session editing a source file that used a NUL as a separator sent that character through in its `act` payload. The server stores a guard context as JSON in a **TEXT** column, so the NUL became a `u0000` escape in that text — and `claimActionExecution` reads three fields back out of it with `d.context::jsonb`, a cast Postgres refuses with 22P05 `unsupported Unicode escape sequence`. The folded claim in `POST /api/guard` 500'd, the hook's `PATCH /api/actions/:id` fallback hit the identical error, and the pretool hook reported "execution claim failed or returned an ambiguous response" and **blocked the tool call**. The same NUL reaching an outcome string failed differently — 22021 `invalid byte sequence for encoding "UTF8": 0x00` on a text parameter — so the outcome PATCH 500'd and left the action at `status='running'`, a dangling row the outcome sweep would later close as a false `lost_confirmation`. Observed on my-dashclaw 2026-09-14 (five blocked calls, five stranded rows) and reproduced exactly on 2026-09-15. Fixed at the write boundary: `app/lib/pg-text.js` strips NULs and unpaired surrogates, `validate()` applies it to every payload that reaches the database (guard context, action record, outcome fields alike), and the execution-claim PATCH — which skips `validate()` — strips its own `act` so the act-content hash still digests the bytes the record stored. The hooks strip the same characters client-side in `encode_json_body`, so an un-upgraded server is protected too and the round trip is never spent on a payload that cannot be stored. (`app/lib/pg-text.js`, `app/lib/validate.js`, `hooks/dashclaw_agent_intel/http_client.py`)
- **The guard's folded claim hashed the raw request body, not the validated one.** `POST /api/guard?record=true` recorded the running action from the validated `data` but passed `body.act` to the claim, so any normalization applied on the way in — now including the strip above — made the claim's act-content hash disagree with the one the record had just stored. (`app/api/guard/route.ts`)
- **A server 5xx was reported to the agent as an ambiguous execution claim.** The claim PATCH ran with `retries=0` and treated every non-answer as a possible half-completed claim, which is what turned the Postgres failure above into five blocked tool calls. One attempt is guaranteed by the *database* — the claim UPDATE gates on `execution_claimed_at IS NULL`, so a second PATCH can only ever answer 409 — so the hook now retries a transient failure (5xx/408/429, or no HTTP status at all) once with a fresh nonce. A refusal the server actually issued still blocks on the first answer. If a response was lost in flight, the hook reads the row back and proceeds only when the claim on record carries one of *its own* attempt ids. (`hooks/dashclaw_pretool.py`)
- **A blocked claim left its action record open, and left no trace.** The hook now cancels the action it abandons via `POST /api/actions/:id/cancel` — the server's own word for "recorded, deliberately never executed", which refuses any already-claimed row — so the entry closes as `cancelled` instead of ageing into a `lost_confirmation` warning that reads as "the agent executed something and never reported it". The failure is also appended to `dashclaw_hook_errors.log`; previously it existed only in the agent's stderr, so the 2026-09-14 incident could only be reconstructed from server logs. (`hooks/dashclaw_pretool.py`)
- **An agent could not cancel its own action.** `POST /api/actions/:actionId/cancel` authorized on `agent_id` only, but a hook or SDK caller's middleware-attributed principal is its **API key id** — the same principal `claimActionExecution` binds a claim to — so the cancel above would have 403'd. Ownership now also matches `created_by`. (`app/api/actions/[actionId]/cancel/route.ts`)
...
- **Catastrophe floor — the 19th policy type, and the first that the calibrated controller cannot reason its way past.** Fires on a destructive action type + risk at or above `min_risk` (default 85) + irreversible, independently of the calibrated threshold θ, and is **ungrantable** by default so an allow grant cannot clear it. It exists because the 2026-09-08 adherence probe proved the controller had calibrated itself out of the ability to interrupt: a synthetic "drop the production database cluster" scored 95, matched no policy, and was **allowed**, while θ had ratcheted to 101.8 — above the score ceiling, so the raise arm could never fire again. A threshold that learns from an approving operator will eventually learn to approve everything; some acts are catastrophic at any θ. Validator, contract renderer, authoring UI and policy-type coverage updated; surface budget 18 → 19 policy types. (`app/lib/guard/policy.ts`, `app/lib/guard/types.ts`)
- **Action cancellation** — `POST /api/actions/:actionId/cancel` atomically cancels a running or pending unclaimed action, requires a redacted and length-capped operator reason, and returns 409 once the action is claimed, executed or terminal. Sets `status='cancelled'` and leaves the outcome pending: a cancelled action has no fabricated result. Audit-logged and published as an action update.
- **`miss_review` — the tightening counterpart to `warn_review`.** `POST /api/calibration/misses` takes `{action_id, label: 'dangerous'|'benign', reason?}` on an action the guard *let through* (409 otherwise), derives the risk score from the persisted action record rather than the client, and audits as `calibration.miss_filed`. A dangerous miss tightens θ at weight 1 and owns its agent; a benign miss moves θ by exactly nothing. `warn_review` stays loosening-only (spec §8 invariant 8) — this does not violate that invariant, it completes it: an operator pointing at one specific allowed act is evidence for more interruption in that class, and it requires a specific act, never a mood. Best-effort ingest — a controller failure answers 503 and never affects the action. (`app/lib/guard/calibration.ts`, `calibration-feedback.ts`, spec `docs/superpowers/specs/2026-09-08-calibration-miss-review.md`)
- **Warn cooldown on `rate_limit` policies** — one warn per cooldown window per policy per agent (default = `window_minutes`, override with `rules.cooldown_minutes`), suppressing **warn only**, never a block or an approval hold. The runaway-agent line had fired 34,760 warns in 30 days; a notice repeated that many times is not a record, it is noise with a database bill.
- **Muse governance integration** — skill (`plugins/dashclaw/skills/muse-governance/`), public guide at `/guides/muse`, `llms.txt` entry and `docs/integrations/muse.md`. **Cooperative, not mechanical**, and the guide says so in those words: the Muse runtime has no pre-tool-call hook, so the agent consults the guard and honors the verdict. That stops the accident class and makes a bypass visible in the ledger; it is not a lock against a determined process at the same privilege. Adherence probing — synthetic held actions the agent must leave pending — is the verification path until the runtime hook lands.
- **Catastrophe-floor regression probe** (`scripts/catastrophe-floor-probe.mjs`) — a scheduled live check that the floor still holds all five irreversible classes (`rm_rf`, `drop_table`, `force_push`, `delete_data`, `delete_branch`) against the running instance. The floor is now proven daily instead of assumed.
- **`self_test` on the guard input** — caller-declared, attribution-only, never gates a decision (the same posture as the attestation fields), and excluded from approval-flood counting. The probe is *designed* to trip policies; on 2026-09-09 its five held cases raised a false "Approval flood — per-action pings paused" banner. A governance product that false-positives daily teaches operators to ignore it. Known limit, accepted: the marker is caller-declared, so a hostile client could set it to dodge flood pings — it only collapses notifications, never approvals, and the ledger still records everything; the follow-up if it matters is a server-side allowlist of self-test agent ids.
